...
class Utils(object):
    ...
    def create_pkgs_tgz(self, dirnames=None):
        ...
        for repo_dir in dirnames:
            ...
            tgz_name = os.path.join('%s' %self.packager_dir, 
    ...
    def update_repodir(self, *pkgcfgs):
        ...
        for pkgcfg in pkgcfgs:
            for pkg in pkgcfg.keys():
                pkgcfg[pkg]['repo'] = os.path.join(self.store, 
                                                   pkgcfg[pkg]['repo'])
    ...
    def copy_built_pkg_files(self, targets=None, extra_dirs=None,
        ...
        for each in targets:
            ...
            for pkg in filter(None, pkgs):      
                ...
                destdirs = self.get_as_list(self.contrail_pkgs[each]['repo'])
                if extra_dirs is not None:
                    extra_dirs = self.get_as_list(extra_dirs)
                    destdirs.extend(extra_dirs)
                self.copyfiles(pkgfile, destdirs)
